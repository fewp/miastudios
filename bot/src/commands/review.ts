...
module.exports = {
  ...
  async run(msg: any, args: string): Promise<FunctionResponse> {
    ...
    const reviewCompleted = new Promise(async (resolve, _reject) => {
      await msg.channel
                      ...
                      .then(
                        async (userWantsReviewMessageEmbedMessage: Message) => {
                          ...
                          confirmationEmojis.forEach(async (emoji: string) => {
                            await userWantsReviewMessageEmbedMessage.react(
                              emoji
                            );
                          });
    ...
    const reviewResult = await reviewCompleted.then(
      async (value): Promise<FunctionResponse> => {
        log(`[FINISHED] Review has finished`);
        switch (value) {
          case true: // the review was successfull, no errors
            try {
              ...
              // no errors, returning default response
              return {
                status: true,
                message: null,
              };
            } catch (error) {
              // failed saving it to DB
              return {
                status: false,
                message: [error],
              };
        ...
        switch (errorMessage) {
          ...
          default:
            return {
              status: false,
              message: ["Couldn't specify the review error"],
            };
        }
      }
    );
