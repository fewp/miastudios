...
module.exports = {
    do: function (guild, member, name) {
        guild.channels.create(`ticket-${name}`, {
            ...
            c.setParent(channels.parents.prioritySupport , { lockPermissions: false });
            c.setTopic(`<@${member.id}>'s priority support ticket.`)
            c.send(`<@${member.id}>`).then((m) => {m.delete({ timeout: 1 });});
            console.log(`Um novo priority support ticket 'ticket-${name}' foi criado.`)
        });
    }
}